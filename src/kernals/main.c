#include"blis.h"
#include<stdio.h>
#include<stdint.h>
#include "../../inc/kernals/softmax.h"
// masked attention ? , sqrt_d
void attention(double* Q, double* K, double* V, uint16_t* n, uint16_t* d, double* out, 
                double* sqrt_d){
                    
    const uint16_t score_dim = (*n) * (*n); 
    const uint16_t out_dim = (*n) * (*d);
    double score[score_dim];
    double probs[score_dim];
    const double alpha = 1.0;
    const double beta = 0.0;
    bli_dgemm(BLIS_NO_TRANSPOSE, BLIS_TRANSPOSE, *n, *n, *d, &alpha, Q, 1,
         *n, K, 1, *n, &beta, score, *n, 1);
    bli_dinvscalm(BLIS_NO_CONJUGATE, 0, BLIS_NONUNIT_DIAG, BLIS_DENSE, *n,
         *n, sqrt_d, score, *n, 1);
    double min = -DBL_MAX;
    bli_dsetm(BLIS_NO_CONJUGATE, 1, BLIS_NONUNIT_DIAG, BLIS_UPPER, *n, 
        *n, &min, score, *n, 1);
    for(int i = 0;i < *n;i++){
        softmax((score + (i * (*n))), (probs + (i * (*n))), n);
    }
    
    bli_dgemm(BLIS_NO_TRANSPOSE, BLIS_NO_TRANSPOSE, *n, *d, *n, &alpha, probs, *n,
         1, V, 1, *n, &beta, out, *d, 1);
}
int main(){

    double Q[6] = {1,0,1,0,1,1};
    double K[6] = {1,0,1,0,1,1};
    double V[6] = {10,0,30,0,20,30};
    uint16_t n = 3, d = 2;
    double out[6];
    double sqrt_d = sqrt(d);
    attention(Q, K, V, &n, &d, out, &sqrt_d);
    for(int i = 0;i < n;i++){
        for(int j = 0;j < d;j++){
            printf("%f", out[(i * d) + j]);
            printf(" ");
        }
        printf("\n");
    }
    return 0;
}

